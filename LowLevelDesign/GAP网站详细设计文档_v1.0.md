...
| **项目**   | **内容**                                                                                                                   |
|------------|----------------------------------------------------------------------------------------------------------------------------|
| 文档定位   | 网站前端与交互的详细设计（LLD）。**本版 v2.3**：以 v2.2（已按文落地，commit `1b558cc`）为 As-Built 基线，追加 1 项**确认态总表改版**——**计划总表卡片化（每日上能力/下动作）+ 客户端图片下载**（§6.5、§7.3、附录 E），凡与既往版本冲突之处一律以本文为准。 |
...
- **确认态**：渲染 `PlanTable`（只读总表，§7.3），工具条含「重新编排」→ 清 `confirmedAt` 回编排态。
  - **v2.3 卡片化**：`PlanTable` 由「横向表格」改为「**每日卡片 · 横向铺排**」——每训练日一张卡片（`.pt-card`），卡片内部**纵向两段**：上段列该日目标能力（能力全称 + 优先级 tier-chip，见 §7.3）；下段纵向平铺该日已选动作行（名称 + 类别徽标 + `组数区间×reps RPE（×f 加量 / 减量轻）`，空态「未选动作」）。卡片不设交互（只读）。
  - **v2.3 图片下载**：工具条新增主按钮「下载计划图片 / Download PNG」（`isDownloading` 防重复点击）。实现方式见 §7.3 `PlanTable`——**纯客户端**渲染图片（无服务器资源）。
- **编排态**（板块结构）：
...
| `PlanTable` | 只读总表（确认态，**v2.3 卡片化**）：**布局 = `.pt-card-grid` 每日卡片横向铺排**（auto-fit，窄屏自然折行）。每张 `.pt-card` 纵向两段——**上段 `.pt-card-targets`**：列该日目标能力（能力全称 + 优先级 tier-chip；无目标 `—`）；**下段 `.pt-card-items`**：纵向平铺已选动作（名称 + 类别徽标 + `lo–hi组×reps RPE（×f 加量 / 减量轻）`；空态「未选动作」）。卡头 = 天 label + 主题（或自由安排）+ 预估 `≈Σminutes·factor` 分钟。尾部 `.pt-foot` 全局行 = practice 单行 + `算法版本 · 生成于时间`。**v2.3 图片下载**：工具条「下载计划图片」按钮 → 取同一卡片网格的**隐藏渲染副本**（`export` 用专用节点，白底固定配色、中英随当前语言），客户端经 `html-to-image` 类库（或 canvas 手绘）合成 PNG → `<a download>` 触发下载（文件名 `gap-plan-yyyymmdd.png`）；纯浏览器端，无服务器资源。`isDownloading` 期间按钮禁用防抖 |
...
| **v2.2** | 2026-09-05 | **1 项引擎优化（需求已确认 → 已按文落地，commit `1b558cc`，2026-09-05 部署）**：① 能力表新增**相斥关系**（§4.2「相斥」列 + §4.1 `Ability.conflicts`）：最小能量系统口径——`有氧系 {AER_CAP, AER_END} × 无氧系 {ANA_CAP, RHIA}` 两两互斥共 4 对，MOB/AWARE 等无相斥；同一训练日目标列表不得并存互斥对。② 每日能力排日改为**「铺开 + 必要重复」两阶段**（§5.8）：阶段一各缺口能力第 1 次暴露尽量各占不同天、跨日不重复；阶段二仅两类必要重复——a) 大缺口 `deficit ≥ 1.2` 强度双频保留，b) 训练日富余（占用天数 < N）时按需填空、上限 `⌈N/A⌉≤3`；配兜底降级（先放宽相斥（仍守容量）→ 满位即顺延，容量不超载；仅 `N=1` 单日整体超载兜底）。单日目标增量按该能力本轮实际出现次数均摊（§5.6）。算法版本 `0.2.1-mock` → `0.3.0-mock`。 |
| **v2.3** | 2026-09-08 | **1 项计划总表改版（需求已确认 → 已按文落地）**：确认态只读总表 `PlanTable` 改版——① **卡片化布局**：由「天为行的横向表格」改为「每日卡片横向铺排」（`.pt-card-grid`）；每卡片内**纵向两段**：上段列该日目标能力（全称 + 优先级 chip），下段纵向平铺动作（名称 + 类别徽标 + `组数×reps RPE`）；卡头含天 label + 主题 + 预估分钟，尾行 practice + 算法版本。② **客户端图片下载**：工具条「下载计划图片」→ 同布局隐藏渲染副本（`html-to-image`）合成 PNG，`<a download>` 触发，文件名 `gap-plan-YYYYMMDD.png`；纯浏览器端无服务器资源；`isDownloading` 防抖。范围：仅确认态，编排态交互不变。 |
